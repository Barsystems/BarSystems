...
import funcionario_funcao.table.FuncionarioFuncaoTableModel;
 ...
 * @author Marcos
 */
public class FuncionarioFuncaoJanela extends JPanel implements ActionListener, KeyListener {
    ...
    private List<FuncionarioFuncaoClasse> func;
    
    private Font fonteTitulo, fonteGeral;
    
    private JTabbedPane painel_principal;
    
    public FuncionarioFuncaoJanela(UsuarioClasse user, int width, JTabbedPane painel_principal) {
        ...
        lblTitulo.setText("MANUTENÇÃO DO CADASTRO DE FUNÇÕES");
        ...
        lblPesquisar = new JLabel("Pesquisar função");
    ...
    public void novo() {
        FuncionarioFuncaoCadastrar cadastrar = new FuncionarioFuncaoCadastrar();
    ...
    public void editar() {
        FuncionarioFuncaoEditar editar = new FuncionarioFuncaoEditar(func.get(tabela.getSelectedRow()));
    ...
    public void excluir() {
        FuncionarioFuncaoClasse funcao = func.get(tabela.getSelectedRow());
        if (new FuncionarioFuncaoExcluir().excluir(funcao) == true) {
            JOptionPane.showMessageDialog(null, "Função "+funcao.getFuncao()+" excluída com sucesso!", "ATENÇÃO", JOptionPane.INFORMATION_MESSAGE);
            refreshTable();
        } else {
            JOptionPane.showMessageDialog(null, "Não foi possível excluir a função "+funcao.getFuncao()+"!", "ATENÇÃO", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    public void refreshTable() {
        func = new FuncionarioFuncaoController().refreshTabela(txtPesquisar.getText());
        if (func != null) {
            tabela.setModel(new FuncionarioFuncaoTableModel(func));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnNovo) {
            ...
            int index = verifica.verificaMenuAberto(painel_principal, "Cadastro de funções");
