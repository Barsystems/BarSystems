...
import cliente.table.ClienteTableModel;
 ...
 * @author Marcos
 */
public class ClienteJanela extends JPanel implements KeyListener, ActionListener {
    ...
    public ClienteJanela(UsuarioClasse user, int width, JTabbedPane painel_principal) {
        ...
        lblTitulo.setText("MANUTENÇÃO DO CADASTRO DE CLIENTES");
        ...
        lblPesquisar = new JLabel("Pesquisar clientes");
    ...
    public void refreshTable() {
        lista = new ClienteController().findCliente(txtPesquisar.getText());
        if (lista != null) {
    ...
    public void novo() {
        try {
            ClienteCadastrar cadastrar = new ClienteCadastrar();
    ...
    public void editar() {
        try {
            ClienteEditar editar = new ClienteEditar(lista.get(tabela.getSelectedRow()));
    ...
    public void excluir() {
        ClienteClasse classe = lista.get(tabela.getSelectedRow());
        ClienteExcluir excluir = new ClienteExcluir();
        boolean result = excluir.excluir(classe);
        if (result == true) {
            JOptionPane.showMessageDialog(null, "Cliente "+classe.getNome()+ " excluído com sucesso!", "ATENÇÃO", JOptionPane.INFORMATION_MESSAGE);
            refreshTable();
        } else {
            JOptionPane.showMessageDialog(null, "Não foi possível excluir o cliente "+classe.getNome()+"!", "ATENÇÃO", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnNovo) {
            ...
            int index = verifica.verificaMenuAberto(painel_principal, "Cadastro de clientes");
