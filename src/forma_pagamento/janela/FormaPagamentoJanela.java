...
import forma_pagamento.table.FormaPagamentoTableModel;
 ...
 * @author Marcos
 */
public class FormaPagamentoJanela extends JPanel implements KeyListener, ActionListener {
    ...
    private List<FormaPagamentoClasse> lista_forma;
    
    private JTabbedPane painel_principal;
    
    public FormaPagamentoJanela(UsuarioClasse user, int width, JTabbedPane painel_principal) {
        ...
        lblTitulo.setText("MANUTENÇÃO DO CADASTRO DE FORMAS DE PAGAMENTO");
        ...
        lblPesquisar = new JLabel("Pesquisar f. de pagto");
    ...
    public void refreshTable() {
        lista_forma = new FormaPagamentoController().findFormaPagamento(txtPesquisar.getText());
        if (lista_forma != null) {
            tabela.setModel(new FormaPagamentoTableModel(lista_forma));
    ...
    public void novo() {
        FormaPagamentoCadastrar cadastrar = new FormaPagamentoCadastrar();
    ...
    public void editar() {
        FormaPagamentoEditar editar = new FormaPagamentoEditar(lista_forma.get(tabela.getSelectedRow()));
    ...
    public void excluir() {
        FormaPagamentoClasse forma = lista_forma.get(tabela.getSelectedRow());
        if (forma.isPadrao() == true) {
            JOptionPane.showMessageDialog(null, "Esta é uma forma de pagamento padrão do sistema e não poderá ser excluída!", "ATENÇÃO", JOptionPane.WARNING_MESSAGE);
        } else {
            FormaPagamentoExcluir excluir = new FormaPagamentoExcluir();
            boolean result = excluir.excluir(forma);
            if (result == true) {
                JOptionPane.showMessageDialog(null, "Forma de pagamento "+forma.getNome()+ " excluída com sucesso!", "ATENÇÃO", JOptionPane.INFORMATION_MESSAGE);
                refreshTable();
            } else {
                JOptionPane.showMessageDialog(null, "Não foi possível excluir a forma de pagamento "+forma.getNome()+"!", "ATENÇÃO", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnNovo) {
            ...
            int index = verifica.verificaMenuAberto(painel_principal, "Cadastro de formas de pagamento");
