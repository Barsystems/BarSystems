...
public class FormaPagamentoDAO implements IFormaPagamentoDAO {
    ...
    private static final String SQL_FIND_ALL = "SELECT forma_pagamento.id_forma_pagamento, forma_pagamento.nome, "
            ...
            + "WHERE forma_pagamento.excluido = 0 ORDER BY forma_pagamento.nome";
    
    private static final String SQL_FIND_FORMA_PAGAMENTO_TIPO_ESPECIFICO = "SELECT forma_pagamento.id_forma_pagamento, "
            + "forma_pagamento.nome, forma_pagamento.id_tipo_fk, forma_pagamento.dias_recebimento, forma_pagamento.padrao, "
            + "forma_pagamento_tipo.nome "
            + "FROM forma_pagamento "
            + "INNER JOIN forma_pagamento_tipo ON forma_pagamento.id_tipo_fk = forma_pagamento_tipo.id_tipo "
            + "WHERE forma_pagamento.excluido = 0 AND forma_pagamento_tipo.nome LIKE ? ORDER BY forma_pagamento.nome";
    ...
    public List<FormaPagamentoClasse> findFormaPagamentoTipoEspecifico(String pesquisa) {
        ...
        try {
            conn = banco.getConexaoMySQL();
            ps = conn.prepareStatement(SQL_FIND_FORMA_PAGAMENTO_TIPO_ESPECIFICO);
