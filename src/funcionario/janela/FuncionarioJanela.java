...
import funcionario.table.FuncionarioTableModel;
 ...
 * @author Marcos
 */
public class FuncionarioJanela extends JPanel implements ActionListener, KeyListener {
    ...
    private List<FuncionarioClasse> func;
    
    private Font fonteTitulo, fonteGeral;
    
    private JTabbedPane painel_principal;
    
    public FuncionarioJanela(UsuarioClasse user, int width, JTabbedPane painel_principal) {
        ...
        lblTitulo.setText("MANUTENÇÃO DO CADASTRO DE FUNCIONÁRIOS");
        ...
        lblPesquisar = new JLabel("Pesquisar funcionário");
    ...
    public void novo() {
        try {
            FuncionarioCadastrar cadastrar = new FuncionarioCadastrar();
    ...
    public void editar() {
        try {
            FuncionarioEditar editar = new FuncionarioEditar(func.get(tabela.getSelectedRow()));
    ...
    public void excluir() {
        FuncionarioClasse funcionario = func.get(tabela.getSelectedRow());
        if (new FuncionarioExcluir().excluir(funcionario) == true) {
            JOptionPane.showMessageDialog(null, "Funcionário "+funcionario.getNome()+" excluído com sucesso!", "ATENÇÃO", JOptionPane.INFORMATION_MESSAGE);
            refreshTable();
        } else {
            JOptionPane.showMessageDialog(null, "Não foi possível excluir o funcionário "+funcionario.getNome()+"!", "ATENÇÃO", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    public void refreshTable() {
        func = new FuncionarioController().findFuncionario(txtPesquisar.getText());
        if (func != null) {
            tabela.setModel(new FuncionarioTableModel(func));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnNovo) {
            ...
            int index = verifica.verificaMenuAberto(painel_principal, "Cadastro de funcionários");
