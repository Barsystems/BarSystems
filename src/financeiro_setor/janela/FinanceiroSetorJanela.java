...
import financeiro_setor.table.FinanceiroSetorTableModel;
 ...
 * @author Marcos
 */
public class FinanceiroSetorJanela extends JPanel implements ActionListener, KeyListener {
    ...
    private List<FinanceiroSetorClasse> lista_setor;
    
    private Font fonteTitulo, fonteGeral;
    
    private JTabbedPane painel_principal;
    
    public FinanceiroSetorJanela(UsuarioClasse user, int width, JTabbedPane painel_principal) {
        ...
        lblTitulo.setText("MANUTENÇÃO DO CADASTRO DE SETORES FINANCEIROS");
    ...
    public void novo() {
        FinanceiroSetorCadastrar cadastrar = new FinanceiroSetorCadastrar();
    ...
    public void editar() {
        FinanceiroSetorClasse setor = lista_setor.get(tabela.getSelectedRow());
        if (setor.isPadrao() == true) {
            JOptionPane.showMessageDialog(null, "Este setor financeiro não poderá ser editado pois é um setor padrão do sistema!", "ATENÇÃO", JOptionPane.WARNING_MESSAGE);
        } else {
            FinanceiroSetorEditar editar = new FinanceiroSetorEditar(setor);
    ...
    public void excluir() {
        FinanceiroSetorClasse setor = lista_setor.get(tabela.getSelectedRow());
        if (setor.isPadrao() == true) {
            JOptionPane.showMessageDialog(null, "Este setor financeiro não poderá ser excluído pois é um setor padrão do sistema!", "ATENÇÃO", JOptionPane.WARNING_MESSAGE);
        } else {
            if (new FinanceiroSetorExcluir().excluir(setor) == true) {
                JOptionPane.showMessageDialog(null, "Setor "+setor.getNome()+" excluído com sucesso!", "ATENÇÃO", JOptionPane.INFORMATION_MESSAGE);
                refreshTable();
            } else {
                JOptionPane.showMessageDialog(null, "Não foi possível excluir o setor "+setor.getNome()+"!", "ATENÇÃO", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    
    public void refreshTable() {
        lista_setor = new FinanceiroSetorController().findSetor(txtPesquisar.getText());
        if (lista_setor != null) {
            tabela.setModel(new FinanceiroSetorTableModel(lista_setor));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == btnNovo) {
            ...
            int index = verifica.verificaMenuAberto(painel_principal, "Cadastro de setores financeiros");
